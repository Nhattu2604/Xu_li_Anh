...
def lowPass_PHBF(self, D0, c, gamma):
    '''Định nghĩa hàm lọc Butterworth dựa trên PHBF'''
    H = np.zeros((self.P, self.Q))
    U0 = int(self.P / 2)
    V0 = int(self.Q / 2)
    for u in range(self.P):
        for v in range(self.Q):
            D_uv = np.sqrt((u - U0) ** 2 + (v - V0) ** 2)
            H[u, v] = 1 / (1 + (D_uv / D0) ** c) ** gamma
    return H


def highPass_Ideals(self, D0):
    '''Định nghĩa hàm lọc highpass Ideals'''
    # H is our filter
    H = np.zeros((self.P, self.Q))
    D = np.zeros((self.P, self.Q))
    U0 = int(self.P / 2)
...
def highPass_Butterworth(self, D0, n):
    '''Định nghĩa hàm lọc highpass Butterworth'''
    H = np.zeros((self.P, self.Q))
    U0 = int(self.P / 2)
    V0 = int(self.Q / 2)
    for u in range(self.P):
...
def bandPass_Butterworth(self, D0_low, D0_high, n):
    '''Định nghĩa hàm lọc Bandpass Butterworth'''
    H = np.zeros((self.P, self.Q))
    U0 = int(self.P / 2)
    V0 = int(self.Q / 2)
    for u in range(self.P):
        for v in range(self.Q):
            D_uv = np.sqrt((u - U0) ** 2 + (v - V0) ** 2)
            H[u, v] = 1 / (1 + ((D_uv**2 - D0_low**2) * (D0_high**2 - D_uv**2)) / (D_uv**2 * (D0_high**2 - D0_low**2)) ** n)
    return H

def lowPass_LHBF(self, D0, c, gamma):
    '''Định nghĩa hàm lọc Butterworth dựa trên LHBF'''
    H = np.zeros((self.P, self.Q))
    U0 = int(self.P / 2)
    V0 = int(self.Q / 2)
    for u in range(self.P):
